...
router.post("/add", async (req, res) => {
  ...
  if (!userId || !company || !status || !dateString) {
    return res
      .status(400)
      .json({ message: "userId, company, status and date are required" });
  }
...
router.get("/summary", async (req, res) => {
  ...
  try {
    ...
    const appliedJobs = await Jobs.countDocuments({
      userId: userId,
      status: "applied",
    });
    const interviewedJobs = await Jobs.countDocuments({
      userId: userId,
      status: "in progress",
    });
    const rejectedJobs = await Jobs.countDocuments({
      userId: userId,
      status: "rejected",
    });
...
router.get("/filter", async (req, res) => {
  ...
  try {
    const filtered = await Jobs.find({ userId: userId, status: status });
